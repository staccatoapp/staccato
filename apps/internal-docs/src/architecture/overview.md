...
| `apps/mobile`                | `@staccato/mobile`            | Expo (React Native) mobile client, built with Expo Router. Implements auth, Home, Library, the audio player (mini player + Now Playing + queue), and Explore (recommendations, unified search, 30s previews, Lidarr requests). | `apps/mobile/src/app/_layout.tsx`         |
