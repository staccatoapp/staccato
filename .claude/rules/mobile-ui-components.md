...
`AlbumArt` (`components/home/album-art.tsx`) is the universal square art primitive and the most-reused component in the app (~15 consumers). It paints a 135° gradient placeholder (centred music glyph + top highlight), swapped for the real artwork via `StaccatoImage` once `artUrl` is available; given **exactly 4** `artUrls` it renders a 2×2 mosaic (the playlist cover convention). It also takes an optional `badge` slot rendered bottom-right inside the art. Don't hand-roll art placeholders — pass a `gradientKey`/`artUrl(s)` to `AlbumArt`.

## Availability badge (single indicator across every art surface)

`AvailabilityBadge` (`components/ui/availability-badge.tsx`) is the one icon-only cloud pill overlaid on album/playlist art (`AlbumArt`'s `badge` slot): downloaded (`CloudCheck`, success-tinted), inLibrary (`Cloud`), partial (`CloudDownload`), recommended (`CloudOff`); `size` is `tile` or `hero`. The state comes from `lib/availability.ts` — pure `albumServerAvailability`/`availabilityFromPending`/`availabilityFromCounts` give the `ServerAvailability`, and `useResolvedAvailability(collectionId, base)` overlays "downloaded" when the collection is fully on-device (online or offline). This replaced the old per-hero text chips; it's the single availability indicator across the album/playlist heroes, `MediaTile` (via an optional `availability` field), the explore rec-playlist carousel, the more-by-artist rail, and the offline grid. Don't reintroduce per-screen chips or derive availability inline.

## Shared MediaTile for album/playlist grid + carousel cells

`components/ui/media-tile.tsx` is the one square art-tile (art over a two-line title/subtitle caption, pressable to open detail) for both the Home carousels (`components/home/carousel.tsx`) and the Library grid (`app/(protected)/library/index.tsx`). It takes a normalized presentational `MediaTileItem` (`{ id, title, subtitle, gradientKey, artUrl?, artUrls?, availability? }`) plus `size` + `onPress`, staying ignorant of the shared zod and Home view-model types — each screen maps its own data into it (Library inline via `pickGradient(id)`; Home in its mappings). Don't reintroduce per-screen tile components. Out of scope: `ArtistCell` (round avatar) and `QuickStartGrid` (horizontal row tile) keep their own layouts; `HeroRec` keeps its richer `HomeRecPlaylist` type.
