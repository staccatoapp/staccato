...
Reachability is a second axis on `useSession`: `connectionStatus: "online" | "offline" | "reconnecting"`, orthogonal to whether we hold credentials (`session`). On launch, `loadInitialSession` keeps the credentials but reports `offline` when the server is unreachable (only a 401 clears the token), so the app enters the protected shell offline instead of bouncing to login; `SessionProvider` then auto-probes `/api/auth/me` every 15s (and on `retryConnection()`), flipping to `online` and `invalidateQueries()`-ing on success. `useAuthedQuery`/`useAuthedInfiniteList` fold `connectionStatus === "online"` into `enabled`, so all reads pause while offline and refetch on recovery — the single seam that degrades every screen without per-screen edits. Offline UI itself lives under `components/offline/` (see [[mobile-offline]]).
